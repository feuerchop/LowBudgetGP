...
%if the expertise of different labelers can efficiently \huang{properly} be assembled. 
however, 
%using 
\huang{employing } a large amount of labelers in the test time can be expensive \huang{in} the long run. For instance, in crowdsourcing we have to pay each labeler in order to get the annotations. In 
% a scenario of 
Internet-of-Things, 
obtaining data from multiple sensors \huang{for direct fusion } 
...
%deliver reliable annotations, \huang{thus we reduce the cost of communication with other labelers effectively}. 
\huang{In this paper}, we 
...
\huang{from multiple annotated training data, where only a small subset of labelers are optimally chosen to reduce communication effort.} We define an iterative optimization procedure where the sparsity of a client vector \huang{is computed} to reweigh the labelers 
%is controlled 
while reducing the overall classification error. 
%Using this procedure, 
\huang{In experiments, we show that our approach 
